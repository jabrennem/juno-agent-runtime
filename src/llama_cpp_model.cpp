...
std::string tool_protocol(const std::span<const ToolDefinition> tools) {
  ...
  for (const ToolDefinition &tool : tools) {
    ...
    definitions.push_back({{"name", tool.name},
...
Expected<GenerationResponse> parse_response(const std::string &text) {
  try {
    ...
    for (const auto &item : parsed.at("tool_calls")) {
      if (!item.contains("name") || !item.at("name").is_string()) {
        return make_unexpected(
            Error{ErrorCode::InvalidModelOutput,
                  "tool call did not include a string name"});
      }
      ToolCall call;
      call.id = item.value(
          "id", "call-" + std::to_string(response.tool_calls.size() + 1));
      call.name = item.at("name").get<std::string>();
      call.arguments_json =
          item.value("arguments", nlohmann::json::object()).dump();
    ...
    return make_unexpected(
        Error{ErrorCode::InvalidModelOutput, exception.what()});
  }
}
...
LlamaCppModel::LlamaCppModel(std::unique_ptr<Impl> impl)
    : impl_(std::move(impl)) {}
LlamaCppModel::~LlamaCppModel() = default;

Expected<std::shared_ptr<LlamaCppModel>>
LlamaCppModel::create(LlamaCppConfig config) {
  if (config.model_path.empty())
    return make_unexpected(Error{ErrorCode::InvalidConfiguration,
                                 "a GGUF model path is required"});
  if (!std::filesystem::exists(config.model_path))
    return make_unexpected(
        Error{ErrorCode::ModelLoadFailed, "GGUF model file does not exist"});
  llama_backend_init();
  auto impl = std::make_unique<Impl>(std::move(config));
  auto params = llama_model_default_params();
  impl->model =
      llama_model_load_from_file(impl->config.model_path.c_str(), params);
  if (!impl->model)
    return make_unexpected(Error{ErrorCode::ModelLoadFailed,
                                 "llama.cpp could not load the GGUF model"});
  try {
    impl->chat_templates = common_chat_templates_init(
        impl->model, impl->config.chat_template_override);
  } catch (const std::exception &exception) {
...
Expected<std::shared_ptr<LlamaCppModel>>
LlamaCppModel::create(const std::string &model_path) {
...
LlamaCppModel::generate(const GenerationRequest &request,
  ...
  if (stop_token.stop_requested())
    return make_unexpected(
        Error{ErrorCode::Cancelled, "generation was cancelled"});
  ...
  for (const Message &message : request.messages) {
    ...
    const Role rendered_role =
        message.role == Role::Tool ? Role::User : message.role;
  ...
  try {
    ...
    if (template_inputs.enable_thinking) {
      template_inputs.chat_template_kwargs["reasoning_effort"] =
          nlohmann::json(reasoning_effort_name(request.reasoning_effort))
              .dump();
    }
    prompt = common_chat_templates_apply(impl_->chat_templates.get(),
                                         template_inputs)
                 .prompt;
  } catch (const std::exception &exception) {
    return make_unexpected(
        Error{ErrorCode::GenerationFailed,
              std::string("llama.cpp could not apply the chat template: ") +
                  exception.what()});
  }
  if (prompt.empty()) {
    return make_unexpected(Error{ErrorCode::GenerationFailed,
                                 "llama.cpp produced an empty chat prompt"});
  }
  ...
  if (tokens.size() + request.config.max_tokens > impl_->config.context_size) {
    return make_unexpected(
        Error{ErrorCode::ContextLimitExceeded,
  ...
  if (llama_decode(context, llama_batch_get_one(
    ...
    return make_unexpected(Error{ErrorCode::GenerationFailed,
                                 "llama.cpp failed to decode the prompt"});
  }
  ...
  llama_sampler_chain_add(sampler,
                          llama_sampler_init_top_p(request.config.top_p, 1));
  llama_sampler_chain_add(sampler,
                          llama_sampler_init_temp(request.config.temperature));
  llama_sampler_chain_add(sampler,
                          llama_sampler_init_dist(request.config.seed));
  struct SamplerGuard {
    llama_sampler *value;
...
Expected<std::shared_ptr<LlamaCppModel>>
LlamaCppModel::create(const std::string &) {
  return make_unexpected(
      Error{ErrorCode::ModelUnavailable,
            "rebuild with JUNO_HARNESS_ENABLE_LLAMA_CPP=ON to use llama.cpp"});
}
Expected<GenerationResponse> LlamaCppModel::generate(const GenerationRequest &,
                                                     const EventCallback &,
                                                     std::stop_token) {
  return make_unexpected(
      Error{ErrorCode::ModelUnavailable,
            "llama.cpp support was not compiled into this build"});
}
} // namespace juno::harness
